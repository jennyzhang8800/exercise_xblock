...
class ExerciseXBlock(XBlock):
    ...
    def addNewExercise(self,data,suffix=''):
        ...
        file_dir=str(int(str(q_number)[:-2])+1)
       
       
        file_parent=os.path.join(path,file_dir)
        ...
        fileIn.close()
        check_path="https://github.com/chyyuu/os_course_exercise_library/tree/master/data/json/"+file_dir+"/"+file_name
        #push to github
        os.system("/var/www/zyni/script/pushToGithub.sh "+file_name)        
        return{'q_number':q_number,"message":check_path}
    @XBlock.json_handler
    def modifyExercise(self,data,suffix=''):
        #pull from github
        os.system("/var/www/zyni/script/pullFromGithub.sh")
